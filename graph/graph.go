...
type LinkerGraph struct {
	...
	TSEnums map[ast.Ref]map[string]ast.TSEnumValue
...
func CloneLinkerGraph(
	...
	for stableIndex, sourceIndex := range reachableFiles {
		...
		go func(sourceIndex uint32) {
			...
			case *JSRepr:
				...
				// Clone the parts
				repr.AST.Parts = append([]ast.Part{}, repr.AST.Parts...)
				for i := range repr.AST.Parts {
					...
					part.Dependencies = append([]ast.Dependency{}, part.Dependencies...)
				}
				...
				namedImports := make(map[ast.Ref]ast.NamedImport, len(repr.AST.NamedImports))
	...
	var tsEnums map[ast.Ref]map[string]ast.TSEnumValue
	...
	for _, sourceIndex := range reachableFiles {
		...
		if repr, ok := file.InputFile.Repr.(*JSRepr); ok && repr.AST.TSEnums != nil {
			if tsEnums == nil {
				tsEnums = make(map[ast.Ref]map[string]ast.TSEnumValue)
			}
...
func (g *LinkerGraph) AddPartToFile(sourceIndex uint32, part ast.Part) uint32 {
...
func (g *LinkerGraph) GenerateRuntimeSymbolImportAndUse(
	...
	runtimeRepr := g.Files[helpers.SourceIndex].InputFile.Repr.(*JSRepr)
	ref := runtimeRepr.AST.NamedExports[name].Ref
	g.GenerateSymbolImportAndUse(sourceIndex, partIndex, ref, useCount, helpers.SourceIndex)
}
